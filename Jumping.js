...
JumpBehavior.prototype = {
    ...
    startJump: function (sprite) {
        if (this.jumping) { // 'jumping' will be reset in finishJump()
    ...
    ascend: function (sprite) {
        var elapsed = this.ascendTimer.getElapsedTime(),
            deltaY = elapsed / (this.duration / 2) * this.height;

        sprite.top = this.jumpStart - deltaY; // Going up

        if (this.ascendTimer.isExpired()) {
    ...
    descend: function (sprite) {
        var elapsed = this.descendTimer.getElapsedTime(),
            deltaY = elapsed / (this.duration / 2) * this.height;

        sprite.top = this.jumpApex + deltaY; // Going down

        if (this.descendTimer.isExpired()) {
            this.descendTimer.stop();
        }
    },

    finishJump: function (sprite) {
        sprite.top = this.jumpStart;
        this.jumping = false;
    },
    ...
    unpause: function () {
        if (this.ascendTimer.isPaused()) {
            this.ascendTimer.unpause();
        }
        else if (this.descendTimer.isPaused()) {
